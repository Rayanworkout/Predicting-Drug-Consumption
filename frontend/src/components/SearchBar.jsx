...
export function SearchBar () {
    const { drugType, setDrugType, drugData, drugTypePrettier, setDrugTypePrettier, chartType } = useStore();
                ...
                <CardContent
                    ...
                    {filteredDATA.length > 0 ? (
                        filteredDATA.map((item, index) => (
                            <p key={index} onClick={() => {handleSelect(item.value), setDrugTypePrettier(item.drug)}}
